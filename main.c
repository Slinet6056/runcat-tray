...
static char* MODES[256] = {"cat", "dab", "mona", "partyblobcat", "zhiyin"};
static int MODES_COUNT = 5;
static int MODE = 0;
...
#define FPS_DELTA (((1000.0 / (double)FPS_L) - (1000.0 / (double)FPS_H)) / SAMPLE_RATE)
...
static gboolean get_cpu_usage() {
    ...
    if (CPU_USAGE.total_b != 0 && CPU_USAGE.total_a != 0) {
        CPU_USAGE.percent = ((double)CPU_USAGE.total_b - (double)CPU_USAGE.total_a) / 1.0 / (double)SAMPLE_RATE /
                            (double)CPU_USAGE.num_cores * 100;
...
int main(int argc, char** argv) {
    ...
    indicator = app_indicator_new(TRAY_APPINDICATOR_ID, FRAMES[0], APP_INDICATOR_CATEGORY_APPLICATION_STATUS);
